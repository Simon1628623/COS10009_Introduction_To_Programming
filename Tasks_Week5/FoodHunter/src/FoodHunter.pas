...
type
	...
	FoodDataArray = array of FoodData; 
	//
	...
	FoodHunterData = record
		...
		food: FoodDataArray;			// There is an array of food
...
begin
	...
	if KeyDown(VK_LEFT) then game.hunter.x -= SPEED;
	if KeyDown(VK_RIGHT) then game.hunter.x += SPEED;
	if KeyDown(VK_UP) then game.hunter.y -= SPEED;
	if KeyDown(VK_DOWN) then game.hunter.y += SPEED;
